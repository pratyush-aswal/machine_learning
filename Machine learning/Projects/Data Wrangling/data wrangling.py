...
def removedirty():
    ...
    for i, j in df.iterrows():
        # Checking if uber ty[pe value is >2
        if j['Uber Type'] > 2:
            if str(j.ID)[2:3] == '1':
    ...
    # Making scatter plots between diff columns so as to check errors present in the data
    ...
    # passing latitude and longitude column values to Latcorrect function
    ...
    # removes dirty dates
    dates = []
    # Cleaning the dates column first as it contains enormous amount of errors
    ...
    for i in range(dates.__len__()):
        ...
        if int(dates[i][2]) > 31:
            dates[i][1], dates[i][2] = dates[i][2], dates[i][1]
        # Chceking validity of the dates also
    ...
    # assigning final dates
    df['Departure Date'] = fd
    # removes dirty time values and check their validity with respect to journey time
    minu = []
    sec = []
    timesd = []
...
# Finding and correcting the outling data values
def outliers():
    ...
    las = np.std(lan)
    los = np.std(lon)
    # Checking the outliers in origin latitude and longitude
    ...
    las = np.std(lan)
    los = np.std(lon)
    # Checking the outliers in destination latitude and longitude
    for i, j in out.iterrows():
...
# Now, dealing with misiing values present in the data file
def misingvalues():
    ...
    # We will remove the missing fares values by updating it with the predicted values
    l = df['Fare$'].isna()
    dg = []
